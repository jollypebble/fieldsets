...
class CircleDiagram extends Component {
  constructor(props) {
    ...
    this.fetchAllSetsData = this.fetchAllSetsData.bind(this);
  ...
  resetState() {
    ...
    // This is the data structure that focus is stored as in the apollo cache ad deined in ./graphql/defaults.js.
    // console.log('Defaults:', defaults);
    const focus = defaults.focus;
    this.props.client.writeData({ data: { focus } });
  ...
  setFocus = () => {
    ...
    const currentSet = this.fetchSet({ id: current.id });
    ...
    const childList = this.fetchAllSetsData({ id: current.id });
  ...
  getFocus = () => {
    return this.props.client.readQuery({ query: getFocus });
  }

  getAllFields = () => {
    return this.props.client.readQuery({ query: fetchAllFields });
  }

  getFieldData = (variables) => {
    const id = `FieldSet:${variables.id}`;
  ...
  getAllSets = () => {
    return this.props.client.readQuery({ query: fetchAllSets });
  }
  ...
  fetchSet = (variables) => {
    ...
    const set = this.props.client.readFragment({
      id,
      fragment: fetchSubSet,
      fragmentName: 'set'
    });
    return set;
  }

  fetchAllSetsData = (variables) => {
    // console.log('Getting cached Sets', variables);
    const id = `SubSet:${variables.id}`;
  ...
  updateFocus = (id, focusX, focusY) => {
    ...
    this.setState({
      currentID: current.focus.id,
      currentX: current.focus.centerX,
      currentY: current.focus.centerY
    });
    this.setFocus();
  }
  ...
  setFieldCache = (data = []) => {
    ...
    data.map((currentField) => {
      ...
      const id = `FieldSet:${currentField.parent}`;

      let fieldSet = this.props.client.readFragment({
        id,
        fragment: fetchFieldset,
        fragmentName: 'fields'
      });
      ...
      fieldSet = (fieldSet === null) ? { id: currentField.parent, list: [], __typename: 'FieldSet' } : fieldSet;
      fieldSet.list = fieldSet.list.filter(item => item.id !== currentField.id);
      fieldSet.list.push(currentField);
  ...
  updateDataCache = (data = []) => {
    ...
    data.map((set) => {
      ...
      set.display.__typename = 'SetView';
      if (set.display.attributes) {
        set.display.attributes.id = setID;
        set.display.attributes.__typename = 'SetViewData';
      }
      if (set.display.zoom) {
        set.display.zoom.id = setID;
        set.display.zoom.__typename = 'Zoom';
      }
      ...
      if (set.parent && set.parent.length) {
        const id = `SubSet:${set.parent}`;
        let subSet = this.props.client.readFragment({
          id,
          fragment: fetchSubSet,
          fragmentName: 'sets'
        });
        // Child cache hasn't been written yet, so set it to defaults.
        subSet = (subSet === null) ? { id: set.id, list: [], __typename: 'SubSet' } : subSet;

        subSet.list.push(set);
  ...
  updateFieldCache(data = []) {
    ...
    this.props.client.mutate({
      ...
      variables: { data },
      refetchQueries: res => res.data.updateField.map(id => ({ query: fetchFields, variables: { id } }))
    });
  }
