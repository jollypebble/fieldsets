...
export const Focus = ( {children} ) => {
  ...
  const setStatus = (newStatus, newMessage = '') => {
    updateStatus({status: newStatus, message: newMessage});
  }

  const setContainer = ( newContainer = false ) => {
    //setStatus('switched', 'Switched to new container focus.');
    if (! newContainer ) {
    ...
    if (call.action) {
      ...
      switch (call.action) {
        ...
        case 'focus':
          ...
          current = mergeFocus(focusdata);
          setStatus('focused', 'Change focus fieldset.');
          break;
        ...
        case 'expand':
          // Fetch our metadata about our new focus.
          current = mergeFocus(focusdata);
          setStatus('expanded', 'Expanded focused fieldset.');
          break;
        default:
          break;
      }
  ...
  const mergeFocus = (data) => {
    ...
    const currentContainer = (container.data) ? { ...container.data.container } :  { ...defaults.focus.container };
